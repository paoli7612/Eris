    ...
    @if ($user->isMe())
        <div id="accordion">
            ...
            <x-dd-div id="collapseLogout">
                <x-q title="logout" collapse="collapseLogout" ask="Are you sure to logout?" action="{{ route('logout') }}"  method="POST" />
            </x-dd-div>
            <x-dd-div id="collapseRem">
                <x-q title="remove" collapse="collapseRem" ask="Are you sure to delete this account?" action="{{ route('account.delete') }}" method="POST"/>
            </x-dd-div>
        </div>
    @endif
